...
@section('content')
    ...
    <div class="glass-card rounded-2xl p-8 shadow-xl max-w-4xl">
        <form action="{{ route('admin.destination-paragraphs.update', $destinationParagraph) }}" method="POST" 
            x-data="{
                selectedCity: '{{ old('filter_city', $destinationParagraph->destination->city_id) }}',
                selectedDest: '{{ old('destination_id', $destinationParagraph->destination_id) }}',
                cityOpen: false,
                destOpen: false,
                cities: [
