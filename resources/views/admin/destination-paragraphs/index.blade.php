...
@section('content')
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-display font-bold text-gray-900 dark:text-white tracking-tight">Destination Content</h2>
            <p class="text-gray-500 dark:text-gray-400 mt-1 text-lg">Manage content paragraphs for destinations.</p>
        </div>
        <a href="{{ route('admin.destination-paragraphs.create') }}" class="btn-primary shadow-lg shadow-primary-500/30">
    ...
    <!-- Filters -->
    <div class="mb-12">
        <form id="search-form" action="{{ route('admin.destination-paragraphs.index') }}" method="GET" class="glass-card rounded-full p-2 flex items-center bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-gray-200/50 dark:border-gray-700/50 shadow-lg w-fit overflow-visible">
            <!-- Search -->
    ...
    <div id="table-container" class="glass-card rounded-2xl overflow-hidden shadow-xl">
        @include('admin.destination-paragraphs.partials.table')
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fetchResults = () => {
                ...
                const url = new URL("{{ route('admin.destination-paragraphs.index') }}");
