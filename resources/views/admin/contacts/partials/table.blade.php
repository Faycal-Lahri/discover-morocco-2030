<table class="w-full">
    ...
    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group cursor-pointer" onclick="window.location='{{ route('admin.contacts.show', $contact) }}'">
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                    {{ $contact->statut === 'traite' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                       ...
                       ($contact->statut === 'non_valide' ? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300' :
            ...
            <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end space-x-3">
                    ...
                    <form action="{{ route('admin.contacts.send-to-workflow', $contact) }}" method="POST" class="inline-block">
                        @csrf
                        <button type="submit" onclick="event.stopPropagation()" class="p-2 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-xl transition-colors" title="Send to Workflow">
                    ...
                    @endif
                    <a href="{{ route('admin.contacts.show', $contact) }}" onclick="event.stopPropagation()" class="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors cursor-pointer" title="View">
                    ...
                    <a href="{{ route('admin.contacts.edit', $contact) }}" onclick="event.stopPropagation()" class="p-2 text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-xl transition-colors" title="Edit">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <form id="delete-form-{{ $contact->id }}" action="{{ route('admin.contacts.destroy', $contact) }}" method="POST" class="inline-block">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="event.stopPropagation(); confirmDelete('delete-form-{{ $contact->id }}')" class="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors" title="Delete">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
