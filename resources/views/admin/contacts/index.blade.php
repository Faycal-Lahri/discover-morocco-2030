...
@section('content')
    <div>
        ...
        <!-- Filters -->
        <div class="mb-12">
            <form id="search-form" action="{{ route('admin.contacts.index') }}" method="GET" class="glass-card rounded-full p-2 flex items-center bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-gray-200/50 dark:border-gray-700/50 shadow-lg w-fit overflow-visible">
                <!-- Search -->
                <div class="relative w-64 flex items-center pl-6">
                    ...
                    <input type="text" id="searchInput" name="search" value="{{ request('search') }}" placeholder="Search messages..." class="w-full bg-transparent border-none p-2 text-sm focus:ring-0 placeholder-gray-500 dark:placeholder-gray-400 text-gray-900 dark:text-white font-medium">
                </div>
                ...
                <div class="relative min-w-[140px]" x-data="{ open: false, label: '{{ request('status') == 'non_lu' ? 'Non Lu' : (request('status') == 'en_cours' ? 'En Cours' : (request('status') == 'traite' ? 'Traité' : 'Status')) }}' }" @click.outside="open = false">
                    ...
                    <div x-show="open" 
                         ...
                         class="absolute left-0 mt-3 w-40 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 z-[100] overflow-hidden text-left origin-top-left">
                        <div class="py-1">
                            <div @click="document.getElementsByName('status')[0].value = ''; label = 'Status'; open = false; document.getElementById('search-form').submit()" class="px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer font-medium">All Status</div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const fetchResults = () => {
                   ...
                   if (form.querySelector('input[name="status"]').value) url.searchParams.set('status', form.querySelector('input[name="status"]').value);
