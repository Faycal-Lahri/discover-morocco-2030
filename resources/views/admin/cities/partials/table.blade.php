                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                            ...
                            <div
                                ...
                                @if($city->image)
                                    @php
                                        ...
                                        if (str_starts_with($city->image, 'http')) {
                                            // External URL (Wikimedia, etc.)
                                            $imageUrl = $city->image;
                                        } elseif (str_starts_with($city->image, 'cities/') || str_starts_with($city->image, 'destinations/')) {
                                            // New storage format (cities/images/...)
                                            $imageUrl = asset('storage/' . $city->image);
                                        } else {
                                            // Old asset format (images/cities/...)
                                            $imageUrl = asset($city->image);
                                        }
                                    @endphp
                                    <img src="{{ $imageUrl }}" alt="{{ $city->nom }}"
                    ...
                    <td class="px-6 py-4 text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-3">
                            ...
                            <form id="delete-form-{{ $city->id }}" action="{{ route('admin.cities.destroy', $city) }}"
                                method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDelete('delete-form-{{ $city->id }}')"
