...
@section('content')
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-display font-bold text-gray-900 dark:text-white tracking-tight">Destinations</h2>
            <p class="text-gray-500 dark:text-gray-400 mt-1 text-lg">Manage tourist destinations and landmarks.</p>
        </div>
        <a href="{{ route('admin.destinations.create') }}" class="btn-primary shadow-lg shadow-primary-500/30">
    ...
    <!-- Filters -->
    <div class="mb-12">
        <form id="search-form" action="{{ route('admin.destinations.index') }}" method="GET" class="glass-card rounded-full p-2 flex items-center bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-gray-200/50 dark:border-gray-700/50 shadow-lg w-fit overflow-visible">
            <!-- Search -->
            <div class="relative w-64 flex items-center pl-6 min-w-[200px]">
                ...
                <input type="text" id="searchInput" name="search" value="{{ request('search') }}" placeholder="Search destinations..." class="w-full bg-transparent border-none p-2 text-sm focus:ring-0 placeholder-gray-500 dark:placeholder-gray-400 text-gray-900 dark:text-white font-medium">
            </div>
            ...
            <!-- City Dropdown -->
            <div class="relative min-w-[140px]" x-data="{ open: false, label: '{{ $cities->firstWhere('id', request('city_id'))?->nom ?? 'City' }}' }" @click.outside="open = false">
                ...
                <div x-show="open" 
                    ...
                    <div class="py-1 max-h-60 overflow-y-auto custom-scrollbar">
                        <div @click="document.getElementsByName('city_id')[0].value = ''; label = 'City'; open = false; document.getElementById('search-form').submit()" 
                        ...
                        @foreach($cities as $city)
                            <div @click="document.getElementsByName('city_id')[0].value = '{{ $city->id }}'; label = '{{ $city->nom }}'; open = false; document.getElementById('search-form').submit()" 
                                 class="px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors font-medium flex items-center">
                                 {{ $city->nom }}
                            </div>
                        @endforeach
    ...
    <div id="table-container" class="glass-card rounded-2xl overflow-hidden shadow-xl">
        @include('admin.destinations.partials.table')
    </div>
