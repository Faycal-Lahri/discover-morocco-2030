...
@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-display font-bold text-gray-900 dark:text-white tracking-tight">Edit Destination</h2>
            <p class="text-gray-500 dark:text-gray-400 mt-1 text-lg">Update details for {{ $destination->nom }}.</p>
        </div>
        ...
        <form action="{{ route('admin.destinations.update', $destination) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nom" id="nom" value="{{ old('nom', $destination->nom) }}" required
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="titre" id="titre" value="{{ old('titre', $destination->titre) }}" required
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="label" id="label" value="{{ old('label', $destination->label) }}"
                            placeholder="e.g. Must Visit" class="pl-10 w-full has-icon">
                    </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="gps_location" id="gps_location"
                            value="{{ old('gps_location', $destination->gps_location) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" placeholder="Write a detailed description..."
                    class="w-full">{{ old('description', $destination->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <!-- Image Upload -->
                <div x-data="{ imagePreview: '{{ $destination->image ? Storage::url($destination->image) : '' }}' }">
            ...
            <div class="md:col-span-2">
                ...
                @if($destination->destinationImages && $destination->destinationImages->count() > 0)
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-4">
                        @foreach($destination->destinationImages as $destImage)
                            <div
                                class="relative group rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700">
                                <div class="aspect-square">
                                    @php
                                        if (str_starts_with($destImage->image, 'http')) {
                                            $imgUrl = $destImage->image;
                                        } elseif (str_starts_with($destImage->image, 'destinations/') || str_starts_with($destImage->image, 'cities/')) {
                                            $imgUrl = asset('storage/' . $destImage->image);
                                        } else {
                                            $imgUrl = asset($destImage->image);
                                        }
                                    @endphp
                                    <img src="{{ $imgUrl }}" alt="{{ $destImage->caption }}" class="w-full h-full object-cover">
                                </div>
                                <div
                                    ...
                                    <div class="absolute bottom-0 left-0 right-0 p-3">
                                        <p class="text-white text-xs font-medium truncate">{{ $destImage->caption }}</p>
                                    </div>
                                </div>
                                <button type="button" onclick="deleteGalleryImage({{ $destImage->id }})"
                                    class="absolute top-2 right-2 p-2 bg-red-500 hover:bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all shadow-lg">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <script>
        function deleteGalleryImage(imageId) {
            ...
            fetch(`/admin_morocco_2030/destination-images/${imageId}`, {
                method: 'DELETE',
                headers: {
